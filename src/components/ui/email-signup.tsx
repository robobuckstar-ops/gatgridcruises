...
export function EmailSignup() {
  ...
  return (
    <div className="bg-[#1E3A5F] rounded-xl p-6 sm:p-8">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-2" role="region" aria-label="Email subscription form">
        ...
        <input
          ...
          required
          disabled={loading}
          className="w-full sm:flex-1 px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder:text-slate-400 text-sm focus:outline-2 focus:outline-offset-2 focus:outline-blue-400 disabled:opacity-50"
        ...
        <button
          type="submit"
          disabled={loading}
          className="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-5 py-3 bg-[#D4AF37] text-[#1E3A5F] font-semibold text-sm rounded-lg hover:bg-yellow-300 transition-colors whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed focus:outline-2 focus:outline-offset-2 focus:outline-blue-400"
        >
