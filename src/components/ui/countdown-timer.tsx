...
import { timeUntilDeparture, type TimeUntilDeparture } from '@/lib/today'
...
export function CountdownTimer({ sailDate, size = 'md' }: CountdownTimerProps) {
  const [timeLeft, setTimeLeft] = useState<TimeUntilDeparture>(() => timeUntilDeparture(sailDate))
  const [mounted, setMounted] = useState(false)

  useEffect(() => {
    setMounted(true)
    setTimeLeft(timeUntilDeparture(sailDate))
    const interval = setInterval(() => setTimeLeft(timeUntilDeparture(sailDate)), 60000)
  ...
  const sizeClassesFor = {
    sm: 'px-2 py-1 text-xs gap-1',
    md: 'px-2.5 py-1.5 text-sm gap-1.5',
    lg: 'px-3 py-2 text-base gap-2',
  }

  // A sailing leaving today is still on sale — Disney books right up to the sail
  // date and `isBookableSailing` keeps it in the listings. Labelling it
  // "Departed" is what made live deals read as expired on the last-minute grid.
  if (timeLeft.status === 'today') {
    return (
      <div
        ...
        aria-label="Sails today. Last chance to book."
      >
        <Clock
          className={cn(
            'shrink-0',
