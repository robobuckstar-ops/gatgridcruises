...
export function SailingCard({ sailing, percentBelow }: SailingCardProps) {
  ...
  // Calculate out-the-door pricing based on selected guest count
        ...
        <div className="space-y-3 pt-3 border-t border-slate-100">
          ...
          <div className="space-y-1">
            <div className="flex items-baseline justify-between">
              ...
              <span className="text-xs text-slate-500">(all-in, {guests === 4 ? '4+' : guests} guest{guests > 1 ? 's' : ''})</span>
            </div>
            <div className="flex items-baseline justify-between">
          ...
          {expandedPrice && (
            <div className="space-y-1.5 px-3 py-2 bg-slate-50 rounded-lg text-xs text-slate-700">
              <div className="flex justify-between">
                ...
                <span className="font-semibold">{formatPrice(sailing.current_lowest_price / guests)}</span>
              </div>
              <div className="flex justify-between">
                <span>Port Fees & Taxes</span>
                <span className="font-semibold">{formatPrice(outTheDoor.total - sailing.current_lowest_price - (sailing.length_nights * 14.5 * guests))}</span>
              </div>
              <div className="flex justify-between">
                <span>Gratuities ({sailing.length_nights} nights × {guests} guests)</span>
                <span className="font-semibold">{formatPrice(sailing.length_nights * 14.5 * guests)}</span>
              </div>
