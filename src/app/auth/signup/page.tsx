  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    try {
      const { error: signupError } = await supabase.auth.signUp({
        ...
        password,
        options: {
          emailRedirectTo: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://gatgridcruises.com'}/auth/callback`,
        },
      })
          ...
          <form onSubmit={handleSignup} className="space-y-4" aria-label="Create account form" noValidate>
            ...
            <div className="mt-6 pt-6 border-t border-slate-200">
              <p className="text-center text-slate-600 text-sm">
                ...
                <Link href="/auth/login" className="text-[#1E3A5F] hover:text-[#162d4a] font-semibold transition-colors">
                  Sign in
                </Link>
              </p>
            </div>
